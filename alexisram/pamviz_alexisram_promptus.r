...
geom_text(data=PAM_tag_epC_non_corr, stat='count', aes(label=..count..), vjust=-0.5, hjust=1.5, color="grey40") +
    ###Good looking parameters
        ...
        scale_x_continuous(breaks=c(7,8,9,10,11,12,13,14,15)) +
        scale_y_continuous(breaks=c(5,10,50,150,1000,4000)) +
        annotate("text", x = 12.25, y = 550, label = "With normal elisions, \u03B31**:", hjust=0) +
        annotate("text", x = 13.85, y = 550, label = non_corr_skewness, hjust=0) +
        annotate("rect", xmin = 11.8, xmax=12.2, ymin= 525, ymax=570, fill="grey40") +
        annotate("text", x = 12.25, y = 500, label = "With custom elisions, \u03B31**:", hjust=0) +
        annotate("text", x = 13.85, y = 500, label = corr_skewness, hjust=0) +
...
#For non-correct df
PAM_md_non_corr <- PAM_tag_epC_non_corr %>%
    group_by(meter) %>%
    summarise(count = n()) %>%
    mutate(rate=sprintf("%0.2f", count/sum(count)*100))

knitr::kable(PAM_md_non_corr, "pipe", align = "lrr")
knitr::kable(PAM_md_non_corr, format = "latex", align = "lrr")
